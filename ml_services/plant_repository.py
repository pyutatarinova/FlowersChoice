...
class PlantRepository:
            ...
            results.append(
                {
                    ...
                    "cosine_similarity": (
                        float(r.get("cosine_similarity")) if r.get("cosine_similarity") is not None else None
                    ),
                }
            )
    ...
    def register_user(
        ...
        with self._get_connection() as conn:
            with conn.cursor() as cur:
                ...
                """,
                    (name, email, encoded_password, created_at, updated_at, json.dumps(features), embedding),
                )
    ...
    def get_user_plants_by_flag(self, user_id: int, flag_column: str) -> List[Dict[str, Any]]:
        ...
        Returns:
            List of plant dicts with id, name, features, score, and notes.
        """
        with self._get_connection() as conn:
            with conn.cursor(cursor_factory=extras.RealDictCursor) as cur:
                cur.execute(
